...
void Mesh::RenderInstancing(GLint posAttribLoc, GLint colAttribLoc, GLint tcAttribLoc, GLint hasTextureLoc, GLuint instanceVBO, GLuint numInstances, GLuint shaderProgram) {
	glBindVertexArray(vao);
	...
	//in a mat4, 4 attribute locations 3-6
	for (int i = 0; i < 4; i++) {
	...
	//texture 
	glUniform1i(hasTextureLoc, m_texture != nullptr);
...
void Mesh::SetupInstanceAttributes(GLuint instanceVBO, GLuint shaderProgram)
{
	//binding vao
	glBindVertexArray(vao);
	glBindBuffer(GL_ARRAY_BUFFER, instanceVBO);

	//mat4 has 4 positions
